"""Helpers communs pour invoquer la CLI `claude` (plan Max) ET `kua` depuis la gateway.

Deux garanties NON-NÉGOCIABLES (revue adversariale) :
1. `claude_env()` : env MINIMAL par ALLOWLIST → AUCUN secret backend (INTERNAL_TOKEN,
   ...
   process enfant. HOME est conservé (auth Max dans ~/.claude) ; pas de clé API → plan Max.
